...
def process_invoice_batch(invoice_data):
    ...
    summary = {
        ...
        "invoices_scheduled": len([r for r in results if r["payment_status"] == "scheduled"]),
        "invoices_pending": len([r for r in results if r["payment_status"] == "pending_approval"]),
        "results": results
    }
    
    print(f"\n💰 Total Amount: ${total_amount:,.2f}")
    print(f"📊 Scheduled: {summary['invoices_scheduled']}, Pending: {summary['invoices_pending']}")
