...
All project governance documents should be maintained in a dedicated `Project_Management` folder in the GitHub repository, separate from the main code documentation:
...
- Keep governance documents separate from technical documentation
- Commit all governance updates alongside code changes
- Use standardized naming conventions for consistent organization
- Include clear dates and version information in all documents

### Project Onboarding for AI Assistants

When starting a new development session with an AI assistant:

1. **Initial Repository Review**: Begin by having the AI assistant review the `Project_Management` folder first to understand:
   - The current project state (from Project Log)
   - Previous session context (from Session Briefs)
   - Implementation methodology (from Implementation Guide)
   - Overall solution architecture (from Solution Guide)

2. **Code Exploration Process**:
   - After reviewing governance documents, have the AI explore the codebase structure
   - Use the Four-Role approach with the AI starting in "Continuity Manager Role"
   - The first task should be to create a summary of current understanding and identify any gaps

3. **Project Continuity Commands**:
   ```
   # Start new AI session with proper context
   # Use this command in the repository root
   cat ./Project_Management/Implementation_Guide.md && find . -type f -name "*.py" -o -name "*.js" | sort
   ```

4. **Session Documentation**:
   - At the beginning of each new session, the AI should create a new Session Brief
   - At the end of each session, the AI should update the Project Log
