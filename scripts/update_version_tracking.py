...
class VersionTracker:
    ...
    def get_current_version(self) -> str:
        ...
        if not self.pyproject_file.exists():
            raise FileNotFoundError(f"pyproject.toml not found at {self.pyproject_file}")
    ...
    def add_new_release(self, version: str, release_type: str, description: List[str], 
        ...
        # Update metadata
        data["metadata"]["last_updated"] = datetime.now(datetime.timezone.utc).isoformat().replace('+00:00', 'Z')
    ...
    def _update_markdown_file(self, data: Dict[str, Any]) -> None:
        ...
        for release in data["releases"]:
            ...
            description_bullets = "<br>".join([f"• {desc}" for desc in release["description"]])
        ...
        content = re.sub(
            r"pip install adri==[\d.]+",
            f"pip install adri=={current_version}",
            content
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update ADRI Validator version tracking")
    parser.add_argument("--validator-repo", default=".", help="Path to ADRI Validator repository")
    parser.add_argument("--standards-repo", required=True, help="Path to ADRI Standards repository")
    parser.add_argument("--release-type", default="Beta", help="Release type (Beta, Stable, RC, etc.)")
    parser.add_argument("--description", nargs="+", help="Release description points")
    parser.add_argument("--breaking-changes", nargs="*", help="Breaking changes (if any)")
