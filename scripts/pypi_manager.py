...
class PyPIManager:
    ...
    def get_pypi_versions(self, include_prereleases: bool = True) -> Dict[str, List[str]]:
        """
    ...
    def get_testpypi_versions(self, include_prereleases: bool = True) -> Dict[str, List[str]]:
        """
    ...
    def _is_prerelease(self, version: str) -> bool:
        ...
        prerelease_patterns = [
            r'.*-alpha.*', r'.*-beta.*', r'.*-rc.*', r'.*-dev.*',
            r'.*a\d+.*', r'.*b\d+.*', r'.*rc\d+.*'
        ]
        return any(re.match(pattern, version, re.IGNORECASE) for pattern in prerelease_patterns)
    ...
    def calculate_next_version(self, current_version: str, change_type: str) -> str:
        ...
        if change_type.lower() not in ["patch", "minor", "major"]:
            raise ValueError(f"Invalid change type: {change_type}. Must be patch, minor, or major")
    ...
    def version_exists_on_pypi(self, version: str, check_testpypi: bool = True) -> Dict[str, bool]:
        """
    ...
    def sync_version_json_with_pypi(self, dry_run: bool = False) -> Dict[str, str]:
        ...
        try:
            ...
            if current_production and version_data.get("current_release") != current_production:
            ...
            if current_testpypi and version_data.get("current_testpypi") != current_testpypi:
            ...
            if not dry_run and updates:
                version_data["metadata"]["last_updated"] = datetime.utcnow().isoformat() + "Z"
    ...
    def get_version_status_report(self) -> Dict:
        ...
        try:
            ...
            sync_status = {
                "production_synced": current_production == version_json_data.get("current_release"),
                "testpypi_synced": current_testpypi == version_json_data.get("current_testpypi"),
                "needs_sync": False
            }
            sync_status["needs_sync"] = not (sync_status["production_synced"] and sync_status["testpypi_synced"])

            # Get next versions
            next_versions = self.calculate_next_versions(current_production) if current_production else {}
            ...
            return {
                ...
                "recommendations": self._get_recommendations(sync_status, current_production, version_json_data),
    ...
    def _get_recommendations(self, sync_status: Dict, current_production: str, version_json_data: Dict) -> List[str]:
        """Generate recommendations based on current status."""
        recommendations = []
        
        if sync_status["needs_sync"]:
            recommendations.append("Run 'python pypi_manager.py --sync' to synchronize VERSION.json with PyPI")
        
        if not current_production:
            recommendations.append("No production version found on PyPI - consider initial release")
        
        if version_json_data.get("current_release") and current_production:
            if version_json_data["current_release"] != current_production:
                recommendations.append(f"VERSION.json shows {version_json_data['current_release']} but PyPI has {current_production}")
        
        if not recommendations:
            recommendations.append("All versions are synchronized - ready for next release")
...
def main():
    ...
    parser.add_argument("--package", default="adri", help="Package name (default: adri)")
    ...
    parser.add_argument("--sync", action="store_true", help="Sync VERSION.json with PyPI")
    parser.add_argument("--dry-run", action="store_true", help="Show what would be synced without making changes")
    parser.add_argument("--next", help="Calculate next version (patch/minor/major)")
    parser.add_argument("--check-version", help="Check if specific version exists on PyPI")
    ...
    try:
        ...
        if args.current:
            ...
            print(f"Sync Status: {'✅ Synchronized' if not status['sync_status']['needs_sync'] else '⚠️ Needs Sync'}")
