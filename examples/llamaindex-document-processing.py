...
try:
    ...
    print("❌ LlamaIndex not installed. Run: python tools/adri-setup.py --framework llamaindex")
    exit(1)
...
def query_knowledge_base(query_data):
    ...
    docs = [Document(text="ADRI provides enterprise data quality validation for AI systems.")]
    index = VectorStoreIndex.from_documents(docs)
    
    # Real LlamaIndex query execution
    query_engine = index.as_query_engine(
        similarity_top_k=query_data["max_results"]
    )
...
def main():
    ...
    try:
        ...
        print(f"✅ Protected: {result['documents_processed']} documents processed safely")
