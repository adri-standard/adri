...
def assess_command(data_path: str, standard_path: str, output_path: Optional[str] = None) -> int:
...
def list_standards_command() -> int:
    ...
    try:
        ...
        if standards_dir.exists():
            yaml_files = list(standards_dir.glob("*.yaml")) + list(standards_dir.glob("*.yml"))
            if yaml_files:
                if standards_found:
                    click.echo()
...
def show_config_command(paths_only: bool = False, environment: Optional[str] = None) -> int:
...
def list_assessments_command(recent: int = 10, verbose: bool = False) -> int:
    ...
    try:
        ...
        if not assessments_dir.exists():
            ...
            click.echo("💡 Run 'adri assess <data> --standard <standard>' to create assessments")
            return 0
...
def show_standard_command(standard_name: str, verbose: bool = False) -> int:
    ...
    try:
        ...
        if os.path.exists(standard_name):
            ...
            standard_path = None
            for path in [f"ADRI/dev/standards/{standard_name}.yaml", 
        ...
        if verbose and "field_requirements" in requirements:
            ...
            for field_name, field_config in field_reqs.items():
                ...
                nullable = "nullable" if field_config.get("nullable", True) else "required"
