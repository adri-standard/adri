...
class TypeInference:
    ...
    def _infer_string_type(self, series: pd.Series) -> str:
        ...
        bool_patterns = sample_values.str.lower().isin(['true', 'false', 'yes', 'no', '1', '0'])
    ...
    def infer_field_constraints(self, series: pd.Series, field_type: str) -> Dict[str, Any]:
        """
    ...
    def _detect_string_pattern(self, series: pd.Series) -> Optional[str]:
        ...
        if sample_values.str.upper().str.match(id_pattern).sum() > len(sample_values) * 0.8:
            return r'^[A-Za-z0-9_\-]+$'
