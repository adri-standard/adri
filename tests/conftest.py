...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "quick: Fast tests that can run frequently")
    config.addinivalue_line(
        "markers", "performance: Performance tests with large datasets"
    )
    config.addinivalue_line("markers", "stress: Stress tests with extreme conditions")
    config.addinivalue_line(
        "markers", "ci_only: Tests that should only run in CI environments"
    )
    config.addinivalue_line("markers", "audit: Tests for audit logging functionality")
