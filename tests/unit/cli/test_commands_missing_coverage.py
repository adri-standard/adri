...
class TestSetupCommandMissingCoverage:
    ...
    def test_setup_command_with_subdirectory_config(self, mock_exists, mock_path, mock_config_manager):
        """Test setup command with config in subdirectory."""
        mock_exists.return_value = False
        mock_config_manager_instance = MagicMock()
        mock_config_manager.return_value = mock_config_manager_instance
        mock_config_manager_instance.create_default_config.return_value = {"test": "config"}
...
class TestAssessCommandMissingCoverage:
    ...
    def test_assess_command_with_verbose_output(self, mock_df, mock_json_dump, mock_file, 
                                              mock_datetime, mock_path, mock_engine, 
    ...
    @patch('adri.cli.commands.json.dump')
    @patch('pandas.DataFrame')
    def test_assess_command_non_verbose_output(self, mock_df, mock_json_dump, mock_file, 
                                             mock_datetime, mock_path, mock_engine, 
                                             mock_load_standard, mock_load_data, mock_config_manager):
...
class TestGenerateStandardMissingCoverage:
    ...
    @patch('pandas.DataFrame')
    def test_generate_standard_with_verbose_debug_info(self, mock_df, mock_yaml_dump, mock_file, 
                                                     mock_exists, mock_path, mock_generator, 
    ...
    @patch('adri.cli.commands.yaml.dump')
    @patch('pandas.DataFrame')
    def test_generate_standard_with_large_dataset_verbose(self, mock_df, mock_yaml_dump, mock_file, 
                                                        mock_exists, mock_path, mock_generator, 
                                                        mock_profiler, mock_load_data, mock_config_manager):
        ...
        mock_profiler_instance.profile_data.return_value = {
            "summary": {"total_rows": 200, "total_columns": 1, "data_types": {"string": 1}}
        }
        ...
        mock_generator_instance.generate_standard.return_value = {
            "standards": {"name": "Test Standard", "id": "test_std", "version": "1.0.0"},
            "requirements": {"overall_minimum": 80.0, "field_requirements": {}, "dimension_requirements": {}}
        }
...
class TestValidateStandardMissingCoverage:
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('adri.cli.commands.json.dump')
    def test_validate_standard_with_output_file(self, mock_json_dump, mock_file, mock_validate):
        ...
        result = validate_standard_command("test.yaml", output_path="validation_report.json")
...
class TestListCommandsMissingCoverage:
    ...
    @patch('adri.cli.commands.os.path.exists')
    @patch('adri.cli.commands.Path')
    def test_list_standards_with_verbose_yaml_error(self, mock_path, mock_exists, mock_config_manager):
        ...
        with patch('builtins.open', mock_open()), \
             patch('adri.cli.commands.yaml.safe_load', side_effect=Exception("YAML error")):
    ...
    @patch('adri.cli.commands.os.path.exists')
    @patch('adri.cli.commands.Path')
    def test_list_training_data_with_parquet_no_pandas(self, mock_path, mock_exists, mock_config_manager):
        ...
        mock_path_instance = MagicMock()
        mock_path.return_value = mock_path_instance
        mock_path_instance.glob.side_effect = [[], [], [mock_file_path]]  # csv, json, parquet
    ...
    @patch('adri.cli.commands.os.path.exists')
    @patch('adri.cli.commands.Path')
    def test_list_assessments_with_recent_limit(self, mock_path, mock_exists, mock_config_manager):
...
class TestCleanCacheMissingCoverage:
    ...
    def test_clean_cache_with_errors(self, mock_path, mock_exists, mock_config_manager):
        ...
        mock_config_manager_instance.get_environment_config.return_value = {
            "paths": {"assessments": "/test", "standards": "/test", "training_data": "/test"}
        }
        mock_exists.return_value = True
    ...
    @patch('adri.cli.commands.os.path.exists')
    @patch('adri.cli.commands.Path')
    def test_clean_cache_with_directory_deletion_error(self, mock_path, mock_exists, mock_config_manager):
        ...
        mock_path_instance = MagicMock()
        mock_path.return_value = mock_path_instance
        mock_path_instance.rglob.side_effect = lambda pattern: [mock_dir] if pattern == "__pycache__" else []
...
class TestExportReportMissingCoverage:
    ...
    def test_export_report_csv_format(self, mock_csv_writer, mock_datetime, mock_json_load, 
                                    mock_file, mock_path, mock_config_manager):
...
class TestShowStandardMissingCoverage:
    ...
    def test_show_standard_with_verbose_field_details(self, mock_yaml_load, mock_file, 
                                                    mock_exists, mock_resolve_path, mock_config_manager):
...
class TestExplainFailureMissingCoverage:
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('adri.cli.commands.json.load')
    def test_explain_failure_with_passed_assessment(self, mock_json_load, mock_file, 
                                                   mock_path, mock_config_manager):
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('adri.cli.commands.json.load')
    def test_explain_failure_with_issues_in_dimensions(self, mock_json_load, mock_file, 
                                                      mock_path, mock_config_manager):
...
class TestDataLoadingMissingCoverage:
    ...
    @patch('builtins.open', new_callable=mock_open)
    def test_load_csv_data_empty_file_with_content_check(self, mock_file, mock_csv_reader):
