...
class TestSetupCommandComprehensive:
    ...
    def test_setup_command_success(self, mock_exists, mock_path, mock_config_manager):
        ...
        assert result == 0
        mock_config_manager_instance.create_default_config.assert_called_once_with("test_project")
    ...
    @patch('adri.cli.commands.Path')
    @patch('adri.cli.commands.os.path.exists')
    def test_setup_command_with_custom_params(self, mock_exists, mock_path, mock_config_manager):
        ...
        assert result == 0
        mock_config_manager_instance.create_default_config.assert_called_once_with("custom_project")
    ...
    @patch('adri.cli.commands.Path')
    @patch('adri.cli.commands.os.path.exists')
    def test_setup_command_creates_parent_directory(self, mock_exists, mock_path, mock_config_manager):
...
class TestAssessCommandComprehensive:
    ...
    def test_assess_command_success(self, mock_df, mock_json_dump, mock_file, mock_datetime, 
                                   mock_path, mock_engine, mock_load_standard, mock_load_data, 
                                   mock_config_manager):
    ...
    def test_assess_command_invalid_environment(self, mock_config_manager):
        ...
        result = assess_command("test_data.csv", "test_standard.yaml", environment="invalid")
    ...
    def test_assess_command_no_data(self, mock_load_data, mock_config_manager):
        ...
        mock_config_manager_instance.get_environment_config.return_value = {
            "paths": {"training_data": "/test", "standards": "/test", "assessments": "/test"}
        }
    ...
    @patch('adri.cli.commands.AssessmentEngine')
    @patch('pandas.DataFrame')
    def test_assess_command_with_performance_limits(self, mock_df, mock_engine, mock_load_standard, 
                                                   mock_load_data, mock_config_manager):
        ...
        with patch('adri.cli.commands.Path'), \
             patch('adri.cli.commands.datetime'), \
...
class TestGenerateStandardCommandComprehensive:
    ...
    @patch('pandas.DataFrame')
    def test_generate_standard_success(self, mock_df, mock_yaml_dump, mock_file, 
        ...
        mock_profiler_instance.profile_data.return_value = {
            "summary": {"total_rows": 100, "total_columns": 2, "data_types": {"string": 2}}
        }
    ...
    @patch('adri.cli.commands.ConfigManager')
    @patch('adri.cli.commands.os.path.exists')
    def test_generate_standard_existing_file_no_force(self, mock_exists, mock_config_manager):
    ...
    @patch('adri.cli.commands.load_data')
    @patch('adri.cli.commands.DataProfiler')
    def test_generate_standard_general_exception(self, mock_profiler, mock_load_data, mock_config_manager):
        ...
        with patch('adri.cli.commands.os.path.exists', return_value=False), \
             patch('pandas.DataFrame'):
