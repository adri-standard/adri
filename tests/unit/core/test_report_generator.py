...
class TestReportGenerator:
    ...
    def test_build_field_analysis(self):
        ...
        assert overall_analysis["ml_readiness"] in ["ready", "needs_cleanup", "not_ready"]
    ...
    def test_generate_complete_report(self):
        ...
        required_sections = ["metadata", "summary", "rule_execution_log", "field_analysis"]
    ...
    def test_validate_report_missing_sections(self):
        ...
        assert validation["valid"] is False
        assert any("Missing required section: rule_execution_log" in error for error in validation["errors"])
        assert any("Missing required section: field_analysis" in error for error in validation["errors"])
    ...
    def test_validate_report_mathematical_inconsistency(self):
        ...
        report["adri_assessment_report"]["summary"]["overall_score"] = 100.0  # Wrong total
        
        validation = self.generator.validate_report(report)
        
        assert validation["valid"] is False
        assert any("Mathematical inconsistency" in error for error in validation["errors"])
