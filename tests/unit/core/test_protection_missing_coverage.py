...
class TestDataProtectionEngineInit:
    ...
    def test_init_with_config_manager_setup(self, mock_standards_loader, mock_config_manager):
...
class TestResolveStandardMissingCoverage:
    ...
    def setup_method(self):
        ...
        with patch('adri.core.protection.ConfigManager'), \
             patch('adri.core.protection.StandardsLoader'):
            self.engine = DataProtectionEngine()

    @patch('adri.core.protection.ConfigManager')
    @patch('adri.core.protection.StandardsLoader')
    def test_resolve_standard_with_custom_pattern(self, mock_standards_loader, mock_config_manager):
        ...
        mock_config_instance.resolve_standard_path_simple.return_value = "/test/custom_func_data.yaml"
        ...
        mock_config_instance.resolve_standard_path_simple.assert_called_with(expected_filename)
    ...
    def test_resolve_standard_with_standard_name(self):
        ...
        self.engine.config_manager.resolve_standard_path_simple.return_value = "/test/custom_standard.yaml"
        
        with patch.object(self.engine, '_try_bundled_standard', return_value=None):
            result = self.engine.resolve_standard("func", "data", standard_name="custom_standard")
        
        self.engine.config_manager.resolve_standard_path_simple.assert_called_with("custom_standard.yaml")
...
class TestTryBundledStandardMissingCoverage:
    ...
    def test_try_bundled_standard_with_custom_standard_name(self):
        ...
        self.engine.standards_loader.standard_exists.assert_called_with("custom_standard")
    ...
    def test_try_bundled_standard_pattern_matching_fallbacks(self):
        ...
        self.engine.standards_loader.load_standard.return_value = {"fallback": "standard"}
        ...
        actual_calls = [call[0][0] for call in self.engine.standards_loader.standard_exists.call_args_list]
    ...
    def test_try_bundled_standard_with_standard_not_found_error(self):
        ...
        self.engine.standards_loader.standard_exists.side_effect = StandardNotFoundError("Not found")
    ...
    def test_try_bundled_standard_with_generic_exception(self):
        ...
        self.engine.standards_loader.standard_exists.side_effect = Exception("Generic error")
...
class TestEnsureStandardExistsMissingCoverage:
    ...
    def test_ensure_standard_exists_file_already_exists(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".yaml", delete=False) as tmp:
            ...
            try:
                result = self.engine.ensure_standard_exists(tmp.name, pd.DataFrame({"col1": [1]}))
                assert result is True
            finally:
                os.unlink(tmp.name)
    ...
    def test_ensure_standard_exists_auto_generate_disabled(self):
        ...
        with pytest.raises(ProtectionError, match="Standard file not found"):
            self.engine.ensure_standard_exists("/nonexistent/standard.yaml", pd.DataFrame({"col1": [1]}))
    ...
    @patch('yaml.dump')
    def test_ensure_standard_exists_generation_with_sampling(self, mock_yaml_dump, mock_file,
                                                           mock_generator_class, mock_profiler_class,
                                                           mock_makedirs):
        ...
        result = self.engine.ensure_standard_exists("/test/new_standard.yaml", large_data)
    ...
    def test_ensure_standard_exists_unsupported_data_type(self, mock_makedirs):
        ...
        with pytest.raises(ProtectionError, match="Cannot generate standard from data type"):
    ...
    def test_ensure_standard_exists_generation_failure(self, mock_profiler_class):
        ...
        with pytest.raises(ProtectionError, match="Failed to generate standard"):
            self.engine.ensure_standard_exists("/test/standard.yaml", pd.DataFrame({"col1": [1]}))
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('yaml.dump')
    def test_ensure_standard_exists_with_list_data(self, mock_yaml_dump, mock_file,
                                                  mock_generator_class, mock_profiler_class,
                                                  mock_makedirs):
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('yaml.dump')
    def test_ensure_standard_exists_with_dict_data(self, mock_yaml_dump, mock_file,
                                                  mock_generator_class, mock_profiler_class,
                                                  mock_makedirs):
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('yaml.dump')
    def test_ensure_standard_exists_with_to_pandas_method(self, mock_yaml_dump, mock_file,
                                                         mock_generator_class, mock_profiler_class,
                                                         mock_makedirs):
...
class TestAssessDataQualityMissingCoverage:
    ...
    def test_assess_data_quality_bundled_standard_caching(self):
        ...
        with patch.object(self.engine, '_generate_data_hash', return_value="hash123"), \
    ...
    def test_assess_data_quality_file_standard_caching(self):
        ...
        with patch.object(self.engine, '_generate_data_hash', return_value="hash456"), \
             patch('adri.core.protection.AssessmentEngine') as mock_assessor_class:
    ...
    def test_assess_data_quality_cache_disabled(self):
        ...
        with patch.object(self.engine, '_generate_data_hash', return_value="hash789"), \
             patch('adri.core.protection.AssessmentEngine') as mock_assessor_class:
...
class TestHandleQualityFailureMissingCoverage:
    ...
    def test_handle_quality_failure_warn_mode(self):
        ...
        with patch.object(self.engine, '_format_quality_error', return_value="Test error message"), \
             patch('adri.core.protection.logger') as mock_logger:
            
            # Should not raise exception, just log warning
            self.engine.handle_quality_failure(mock_result, "warn", 80.0, "/test/standard.yaml")
            
            mock_logger.warning.assert_called_once_with("Data quality warning: Test error message")
    ...
    def test_handle_quality_failure_continue_mode(self):
        ...
        with patch.object(self.engine, '_format_quality_error', return_value="Test error message"), \
            ...
            self.engine.handle_quality_failure(mock_result, "continue", 80.0, "/test/standard.yaml")
            
            mock_logger.debug.assert_called_once_with("Data quality failure (continuing): Test error message")
...
class TestProtectFunctionCallMissingCoverage:
    ...
    def test_protect_function_call_with_dimension_requirements(self):
        ...
        with patch.object(self.engine, '_extract_data_parameter', return_value=test_data), \
             patch.object(self.engine, 'resolve_standard', return_value="/test/standard.yaml"), \
             ...
             patch.object(self.engine, '_check_dimension_requirements') as mock_check_dims, \
             patch.object(self.engine, '_format_quality_success', return_value="Success message"):
            
            result = self.engine.protect_function_call(
                test_func, (test_data,), {}, "data", "test_func",
                dimensions=dimensions, verbose=True
            )
    ...
    def test_protect_function_call_with_bundled_standard_verbose(self):
        ...
        test_data = pd.DataFrame({"col1": [1, 2, 3]})
        bundled_standard = {"standards": {"id": "test_standard", "name": "Test Standard"}}
        mock_result = MagicMock()
        mock_result.overall_score = 85.0
        
        with patch.object(self.engine, '_extract_data_parameter', return_value=test_data), \
             patch.object(self.engine, 'resolve_standard', return_value=bundled_standard), \
             patch.object(self.engine, 'ensure_standard_exists', return_value=True), \
             patch.object(self.engine, 'assess_data_quality', return_value=mock_result), \
             patch.object(self.engine, '_format_quality_success', return_value="Success message"), \
             patch('adri.core.protection.logger') as mock_logger:
...
class TestExtractDataParameterMissingCoverage:
    ...
    def test_extract_data_parameter_inspection_failure(self):
        ...
        with patch('inspect.signature', side_effect=Exception("Inspection failed")), \
            ...
            with pytest.raises(ValueError, match="Could not find data parameter 'data'"):
                self.engine._extract_data_parameter(test_func, args, kwargs, "data")
            
            mock_logger.warning.assert_called_once_with("Could not inspect function signature: Inspection failed")
    ...
    def test_extract_data_parameter_not_found(self):
        ...
        with pytest.raises(ValueError, match="Could not find data parameter 'missing_param'"):
            self.engine._extract_data_parameter(test_func, args, kwargs, "missing_param")
...
class TestFormatQualityErrorMissingCoverage:
    ...
    def test_format_quality_error_with_bundled_standard(self):
        ...
        with patch.object(self.engine, '_identify_main_issues', return_value="invalid data formats detected"):
            error_msg = self.engine._format_quality_error(mock_result, 80.0, bundled_standard)
    ...
    def test_format_quality_error_with_file_standard(self):
        ...
        with patch.object(self.engine, '_identify_main_issues', return_value="quality threshold not met"):
            error_msg = self.engine._format_quality_error(mock_result, 85.0, standard_path)
...
class TestIdentifyMainIssuesMissingCoverage:
    ...
    def test_identify_main_issues_single_problem(self):
        ...
        assert result == "invalid data formats detected (e.g., bad emails, invalid dates)"
    ...
    def test_identify_main_issues_many_problems(self):
        ...
        assert "invalid data formats detected" in result and "missing required data fields" in result
...
class TestFormatQualitySuccessMissingCoverage:
    ...
    def test_format_quality_success_bundled_standard_verbose(self):
        ...
        bundled_standard = {
            "standards": {"id": "test_standard", "name": "Test Standard", "version": "2.0.0"}
        }
        
        with patch.object(self.engine, '_was_standard_just_created', return_value=False):
    ...
    def test_format_quality_success_new_standard_verbose(self):
        ...
        with patch.object(self.engine, '_was_standard_just_created', return_value=True), \
             patch('os.path.exists', return_value=True), \
             patch('builtins.open', mock_open(read_data='standards:\n  version: "1.5.0"')), \
...
class TestCheckDimensionRequirementsMissingCoverage:
    ...
    def test_check_dimension_requirements_missing_dimension_raise(self):
        ...
        with pytest.raises(ProtectionError, match="Required dimension 'missing_dimension' not found"):
            self.engine._check_dimension_requirements(mock_result, dimensions, "raise")
    ...
    def test_check_dimension_requirements_insufficient_score_raise(self):
        ...
        with pytest.raises(ProtectionError, match="Dimension 'validity' score insufficient"):
            self.engine._check_dimension_requirements(mock_result, dimensions, "raise")
    ...
    def test_check_dimension_requirements_insufficient_score_warn(self):
        ...
        mock_result = MagicMock()
        mock_result.dimension_scores = {
            "completeness": MagicMock(score=10.0)
        }
    ...
    def test_check_dimension_requirements_continue_mode_silent(self):
        ...
        with patch('adri.core.protection.logger') as mock_logger:
            ...
            self.engine._check_dimension_requirements(mock_result, dimensions, "continue")
