...
class TestCompleteWorkflowIntegration(unittest.TestCase):
    ...
    def test_guided_workflow_from_subdirectory(self):
        ...
        # Step 1: Setup from subdirectory (should work with path resolution)
        result = setup_command(guide=True, project_name="subdir_workflow_test")
        self.assertEqual(result, 0)
        
        # Verify structure was created at project root
        config_path = project_root / "ADRI" / "config.yaml"
        self.assertTrue(config_path.exists())
        
        tutorial_file = project_root / "ADRI" / "tutorials" / "invoice_processing" / "invoice_data.csv"
        self.assertTrue(tutorial_file.exists())
        
        # Step 2: Generate standard from subdirectory
        with patch('adri.cli.load_data') as mock_load_data:
            mock_load_data.return_value = [
                {"invoice_id": "INV-001", "amount": 1250.00, "status": "paid"}
            ]
        ...
        standard_path = project_root / "ADRI" / "dev" / "standards" / "invoice_data_ADRI_standard.yaml"
        self.assertTrue(standard_path.exists())
    ...
    def test_workflow_with_mock_projects(self):
        ...
        with mock_project_context(MockProjectFixtures.complex_multi_directory_project(), "docs/src") as project_root:
            ...
            self.assertEqual(root_from_subdir, project_root)
            ...
            self.assertTrue("ADRI/tutorials" in str(tutorial_path))
            self.assertTrue("ADRI/dev" in str(dev_path))
...
class TestWorkflowIntegration(unittest.TestCase):
    ...
    def test_workflow_with_environment_documentation_integration(self):
        ...
        # Verify comprehensive config documentation was created
        config_path = Path("ADRI/config.yaml")
        with open(config_path, 'r') as f:
            config_content = f.read()
        
        # Check for environment documentation sections
        doc_sections = [
