...
class DemoValidator:
    ...
    def discover_examples(self) -> List[Path]:
        ...
        for file_path in self.examples_dir.glob("*.py"):
            if file_path.name.startswith(("langchain-", "crewai-", "autogen-", 
                                         "llamaindex-", "haystack-", "langgraph-", 
                                         "semantic-kernel-")):
    ...
    def validate_problem_recognition(self, example_path: Path) -> Tuple[bool, str]:
        ...
        try:
            ...
            problem_indicators = [
                "GitHub issue", "github.com", "real-world problem", "production issue",
                "data validation", "conversation flow", "agent coordination", 
                "document processing", "knowledge retrieval", "workflow automation",
                "customer service", "business analysis", "research collaboration"
            ]
            
            found_indicators = [indicator for indicator in problem_indicators 
    ...
    def validate_solution_credibility(self, example_path: Path) -> Tuple[bool, str]:
        ...
        try:
            ...
            adri_patterns = [
                "@adri_protected", "adri_protected", "ADRI", "data quality",
                "protection", "validation", "standard"
            ]
            
            found_patterns = [pattern for pattern in adri_patterns 
            ...
            if "@adri_protected" in content or "adri_protected" in content:
                ...
                if 1 <= decorator_count <= 3:
                    return True, f"Natural ADRI integration ({decorator_count} decorators)"
                elif decorator_count > 3:
                    return False, f"Over-engineered ({decorator_count} decorators - too many)"
                else:
    ...
    def validate_value_clarity(self, example_path: Path) -> Tuple[bool, str]:
        ...
        try:
            ...
            value_indicators = [
                ...
                "error", "failure", "issue", "problem", "solution", "benefit"
            ]
            ...
            if len(value_lines) >= 3:
                return True, f"Clear value messaging found ({len(value_lines)} value statements)"
            else:
                return False, f"Value proposition not clear enough ({len(value_lines)} value statements)"
    ...
    def validate_workflow_naturalness(self, example_path: Path) -> Tuple[bool, str]:
        ...
        try:
            ...
            framework_patterns = {
                "langchain": ["ChatOpenAI", "LLMChain", "PromptTemplate", "chat", "llm"],
                "crewai": ["Agent", "Task", "Crew", "crew.kickoff", "agent"],
                "autogen": ["ConversableAgent", "GroupChat", "GroupChatManager", "chat"],
                "llamaindex": ["VectorStoreIndex", "SimpleDirectoryReader", "query_engine", "index"],
            ...
            if len(found_patterns) >= 2:
                return True, f"Natural {framework} usage: {', '.join(found_patterns[:3])}"
            else:
    ...
    def validate_setup_tool_integration(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            professional_features = [
                "virtual environment", "venv", "dependency", "install", 
                "framework", "requirements", "python -m pip"
            ]
            
            found_features = [feature for feature in professional_features 
                            if feature.lower() in content.lower()]
            
            if len(found_features) >= 4:
                return True, f"Professional setup tool with {len(found_features)} features"
            else:
                return False, f"Setup tool lacks professional features ({len(found_features)} found)"
                
        except Exception as e:
            return False, f"Error reading setup tool: {e}"
    
    def test_example_execution(self, example_path: Path, timeout: int = 10) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if result.returncode in [0, 1]:
                ...
                if "ADRI" in output or "adri" in output:
                    return True, f"Executed successfully (exit code {result.returncode})"
                else:
    ...
    def validate_demo_credibility(self, example_path: Path) -> Dict[str, any]:
        ...
        results["credible"] = results["overall_score"] >= 70.0  # 70% threshold for credibility
