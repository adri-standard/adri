...
class TestFirstImpression:
    ...
    def setup_class(cls):
        ...
        print("=" * 65)
        print(
            "🎯 Goal: AI engineer says 'This solves my real problem' within 30 seconds"
        )
    ...
    def test_immediate_problem_recognition(self):
        ...
        for example_path in self.examples:
            ...
            if len(found_patterns) == 0:
                ...
                print(
                    f"✅ {framework.upper()}: Immediate problem recognition - {found_patterns[0]}"
                )

        if unrecognizable_problems:
            failure_msg = "\n".join(
                [f"   • {prob}" for prob in unrecognizable_problems]
            )
    ...
    def test_value_proposition_visibility(self):
        ...
        for example_path in self.examples:
            ...
            found_value = [
                phrase for phrase in value_phrases if phrase.lower() in header.lower()
            ]
            ...
            if len(found_value) < 2:
                ...
                print(
                    f"✅ {framework.upper()}: Clear value in header - {found_value[:2]}"
                )
    ...
    def test_professional_first_appearance(self):
        ...
        for example_path in self.examples:
            ...
            found_professional = [
                ind for ind in professional_indicators if ind in content
            ]
            ...
            found_unprofessional = [
                ind
                for ind in unprofessional_indicators
    ...
    def test_realistic_business_context(self):
        ...
        for example_path in self.examples:
            ...
            found_business = [
                ctx for ctx in business_contexts if ctx.lower() in content.lower()
            ]
            ...
            toy_count = sum(
                content.lower().count(toy.lower()) for toy in toy_indicators
            )
    ...
    def test_easy_entry_point(self):
        ...
        for example_path in self.examples:
            ...
            if len(found_entry) < 2 and not has_shebang:
                ...
                confusing_entry.append(f"{framework}: Missing executable shebang")
            else:
    ...
    def test_adri_prominence_without_overwhelming(self):
        ...
        for example_path in self.examples:
            ...
            if adri_mentions == 0:
                ...
                print(
                    f"✅ {framework.upper()}: Good ADRI balance ({proportion:.1f}% of content)"
                )
...
class TestThirtySecondExperience:
    ...
    def test_header_tells_complete_story(self):
        ...
        for example_path in self.examples:
            ...
            has_problem = any(
                word in header_text
                for word in ["problem", "issue", "challenge", "fails", "error"]
            )
            has_solution = any(
                word in header_text
                for word in ["adri", "protection", "solution", "prevents", "solves"]
            )
            has_value = any(
                word in header_text
                for word in ["quality", "reliable", "validation", "benefit"]
            )
            has_context = any(
                word in header_text
                for word in ["customer", "business", "production", "real-world"]
            )
            ...
            if story_score < 3:
                ...
                print(
                    f"✅ {framework.upper()}: Complete story in header ({story_score}/4 elements)"
                )
    ...
    def test_framework_authenticity_immediate(self):
        ...
        if inauthentic_usage:
            ...
            pytest.fail(
                f"Examples don't look authentically framework-specific:\n{failure_msg}"
            )
...
class TestFirstImpressionMetrics:
    ...
    def test_overall_first_impression_score(self):
        ...
        for example_path in self.examples:
            ...
            if any(
                word in first_lines
                for word in ["problem", "issue", "validation", "failure", "error"]
            ):
            ...
            if (
                "#!/usr/bin/env python3" in content
                and '"""' in content
                and "if __name__" in content
            ):
            ...
            if (
                sum(1 for word in business_words if word.lower() in content.lower())
                >= 2
            ):
            ...
            status = (
                ...
                if framework_score >= 80
                    ...
                    else "⚠️ NEEDS WORK" if framework_score >= 60 else "❌ POOR"
                )
            )
