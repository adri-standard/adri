...
class TestDemoCredibility:
    ...
    def test_all_examples_have_credible_solutions(self):
        ...
        for example_path in self.examples:
            is_credible, reason = self.validator.validate_solution_credibility(
                example_path
            )
    ...
    def test_all_examples_feel_natural(self):
        ...
        for example_path in self.examples:
            is_natural, reason = self.validator.validate_workflow_naturalness(
                example_path
            )
    ...
    def test_overall_demo_credibility_score(self):
        ...
        for example_path in self.examples:
            ...
            if credible:
                ...
                for validation_name, validation_result in results[
                    "validations"
                ].items():
        ...
        overall_credibility = (
            (credible_count / total_examples) * 100 if total_examples > 0 else 0
        )
...
class TestFrameworkSpecificCredibility:
    ...
    def test_langchain_demonstrates_conversation_issues(self):
        ...
        for example_path in langchain_examples:
            ...
            found_problems = [
                p for p in langchain_problems if p.lower() in content.lower()
            ]

            assert (
                len(found_problems) >= 3
    ...
    def test_autogen_demonstrates_coordination_issues(self):
        ...
        for example_path in autogen_examples:
            ...
            found_problems = [
                p for p in autogen_problems if p.lower() in content.lower()
            ]
    ...
    def test_crewai_demonstrates_workflow_issues(self):
        ...
        for example_path in crewai_examples:
            ...
            found_problems = [
                p for p in crewai_problems if p.lower() in content.lower()
            ]
...
class TestDemoUserExperience:
    ...
    def test_demo_setup_to_execution_workflow(self):
        ...
        execution_failures = []
        for example_path in self.examples:
            executed_ok, reason = self.validator.test_example_execution(
                example_path, timeout=15
            )
            if not executed_ok:
        ...
        print(
            f"✅ Complete demo workflow validated for {len(self.examples)} frameworks"
        )
    ...
    def test_demo_problem_solution_clarity(self):
        ...
        for example_path in self.examples:
            ...
            has_early_problem = any(
                word in first_third.lower() for word in problem_words
            )
            ...
            if not (has_early_problem and has_solution):
                framework = self.validator.extract_framework_name(example_path)
                unclear_demos.append(
                    f"{framework}: Missing clear problem→solution flow"
                )
