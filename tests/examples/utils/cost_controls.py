...
class CostController:
    ...
    def can_make_call(self, framework: str, test_name: str, estimated_cost: float = 0.01) -> tuple[bool, str]:
        """
        ...
        if len(framework_calls) >= self.max_calls_per_framework:
            return False, f"Maximum calls for {framework} ({self.max_calls_per_framework}) reached"
        ...
        if total_cost + estimated_cost > self.max_estimated_cost:
            return False, f"Cost limit (${self.max_estimated_cost:.2f}) would be exceeded"
    ...
    def record_call(self, framework: str, test_name: str, estimated_cost: float = 0.01, 
    ...
    def get_cost_summary(self) -> Dict:
        ...
        for framework in framework_stats:
            framework_records = [r for r in self.call_records if r.framework == framework]
            successful = len([r for r in framework_records if r.success])
            framework_stats[framework]['success_rate'] = successful / len(framework_records)
    ...
    def print_cost_summary(self) -> None:
        ...
        print("=" * 50)
        print(f"📞 Total calls: {summary['total_calls']}/{summary['limits']['max_total_calls']}")
        ...
        if summary['framework_stats']:
            ...
            for framework, stats in summary['framework_stats'].items():
                ...
                print(f"  {framework:15} | {stats['calls']:2d}/{calls_limit} calls | "
                      f"${stats['cost']:.3f} | {success_pct:3.0f}% success")
        
        print("=" * 50)


def estimate_openai_cost(model: str = "gpt-3.5-turbo", input_tokens: int = 100, 
                        output_tokens: int = 50) -> float:
    """
