...
class ADRIDecoratorVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        for decorator in node.decorator_list:
            ...
            elif isinstance(decorator, ast.Attribute) and decorator.attr == 'adri_protected':
                self.adri_protected_functions.append(node.name)
...
class TestADRIDecorators:
    ...
    def test_langchain_example_has_decorators(self):
        ...
        assert len(visitor.adri_protected_functions) > 0, "Should have @adri_protected functions"
        
        # Check for specific expected functions
        expected_functions = ['customer_service_agent', 'langchain_qa_pipeline', 'langchain_conversation_chain']
        found_protected = [f for f in expected_functions if f in visitor.adri_protected_functions]
        assert len(found_protected) > 0, f"Should have protected functions from: {expected_functions}"
    ...
    def test_crewai_example_has_decorators(self):
        ...
        expected_functions = ['analyze_market_segment', 'crewai_customer_support_crew', 'crewai_content_creation_crew']
        found_protected = [f for f in expected_functions if f in visitor.adri_protected_functions]
        assert len(found_protected) > 0, f"Should have protected functions from: {expected_functions}"
    ...
    def test_autogen_example_has_decorators(self):
        ...
        expected_functions = ['conduct_research_project', 'autogen_coding_collaboration', 'autogen_problem_solving_team']
        found_protected = [f for f in expected_functions if f in visitor.adri_protected_functions]
        assert len(found_protected) > 0, f"Should have protected functions from: {expected_functions}"
    ...
    def test_haystack_example_has_decorators(self):
        ...
        expected_functions = ['document_search_pipeline', 'haystack_qa_pipeline', 'haystack_summarization']
        found_protected = [f for f in expected_functions if f in visitor.adri_protected_functions]
        assert len(found_protected) > 0, f"Should have protected functions from: {expected_functions}"
    ...
    def test_llamaindex_example_has_decorators(self):
        ...
        expected_functions = ['process_rag_query', 'llamaindex_document_ingestion', 'llamaindex_semantic_search']
        found_protected = [f for f in expected_functions if f in visitor.adri_protected_functions]
        assert len(found_protected) > 0, f"Should have protected functions from: {expected_functions}"
    ...
    def test_langgraph_example_has_decorators(self):
        ...
        expected_functions = ['execute_analysis_workflow', 'langgraph_chatbot_workflow', 'langgraph_decision_workflow']
        found_protected = [f for f in expected_functions if f in visitor.adri_protected_functions]
        assert len(found_protected) > 0, f"Should have protected functions from: {expected_functions}"
    ...
    def test_semantic_kernel_example_has_decorators(self):
        ...
        expected_functions = ['execute_ai_function', 'semantic_kernel_planning_function', 'semantic_kernel_memory_function']
        found_protected = [f for f in expected_functions if f in visitor.adri_protected_functions]
        assert len(found_protected) > 0, f"Should have protected functions from: {expected_functions}"
...
class TestADRIImports:
    ...
    def test_all_examples_import_adri_protected(self):
        ...
        for filename in example_files:
            ...
            assert 'adri_protected' in content, f"{filename} should import adri_protected"
...
class TestDecoratorUsage:
    ...
    def test_decorators_are_used_correctly(self):
        ...
        for filename in example_files:
            ...
            assert decorator_count > 0, f"{filename} should use @adri_protected decorator"
            
            # Should have reasonable number of decorators (not excessive)
            assert decorator_count <= 10, f"{filename} should not have excessive decorators ({decorator_count})"
        ...
        for filename in example_files:
            ...
            for func_name in visitor.all_functions:
                if any(keyword in func_name.lower() for keyword in 
                      ['workflow', 'pipeline', 'agent', 'process', 'execute', 'run']):
                    likely_main_functions.append(func_name)
            
            if likely_main_functions:
                # At least some of these should be protected
                protected_main = [f for f in likely_main_functions if f in visitor.adri_protected_functions]
                assert len(protected_main) > 0, \
