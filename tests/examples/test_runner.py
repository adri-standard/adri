...
from tests.examples.utils.cost_controls import get_cost_controller, reset_cost_controller
...
class ExamplesTestRunner:
    ...
    def run_smoke_tests(self, frameworks: Optional[List[str]] = None):
        ...
        total_problems = sum(int(framework_problems[f].split("+")[0]) for f in frameworks if f in framework_problems)
        
        print(f"\n🚨 PROBLEMS PREVENTED ACROSS {len(frameworks)} FRAMEWORKS:")
        for framework in frameworks:
            if framework in framework_problems:
                print(f"   • {framework.title()}: {framework_problems[framework]} → 0 failures with ADRI")
        ...
        print(f"💸 ESTIMATED COST SAVINGS: ${total_problems * 12:.2f} per project (at $50/hour)")
        
        import pytest
        
        # Run smoke tests
        test_args = [
            "tests/examples/smoke_tests/",
            "-v",
            "--tb=short"
        ]
        ...
        try:
            ...
            ] + test_args, capture_output=True, text=True, cwd=".")
            ...
            if tests_passed:
                ...
                for framework in frameworks:
                    if framework in community_impact:
                        print(f"   📽️  {framework.title()}: {community_impact[framework]}")
    ...
    def run_error_handling_tests(self, frameworks: Optional[List[str]] = None):
        ...
        print(f"🔍 VALIDATING ERROR HANDLING: {', '.join(f.title() for f in frameworks)}")
            ...
            if exit_code == 0:
                ...
                for framework in frameworks:
                    if framework in enterprise_readiness:
                        print(f"   🏆 {framework.title()}: {enterprise_readiness[framework]}")
                ...
                print("❌ Some error handling tests failed - production readiness at risk")
            ...
            if exit_code == 0:
                ...
                for framework in frameworks:
                    if framework in onboarding_impact:
                        print(f"   ⚡ {framework.title()}: {onboarding_impact[framework]}")
    ...
    def run_interactive(self):
        ...
        while True:
            ...
            if choice == 1:  # Smoke tests
                ...
                framework_choice = self.get_user_choice("Select framework", len(self.available_frameworks) + 2)
...
def main():
    ...
    parser.add_argument("--dependency", action="store_true", help="Run dependency validation tests")
