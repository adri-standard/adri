...
class TestVirtualEnvironmentDetection:
    ...
    def test_virtual_environment_detected_scenario(self):
                        ...
                        try:
                            ...
                            if example_file.exists():
                                ...
                                if pattern_matches >= 3:  # Most patterns found
                                    ...
                                    print(f"  ✅ {framework_name}: Virtual environment detection implemented")
                                else:
                                    print(f"  ⚠️ {framework_name}: Virtual environment detection missing or incomplete")
                            else:
        ...
        detection_rate = (correct_detections / frameworks_tested) * 100 if frameworks_tested > 0 else 0
        ...
        assert detection_rate >= 80.0, f"Venv detection implementation rate too low: {detection_rate}%"
    ...
    def test_virtual_environment_not_detected_scenario(self):
                    ...
                    with patch.object(sys, 'real_prefix', None, create=True):
                        ...
                        try:
                            ...
                            if example_file.exists():
                                ...
                                if pattern_matches >= 3:  # Most patterns found
                                    correct_warnings += 1
                                    print(f"  ✅ {framework_name}: Virtual environment warning implemented")
                                else:
                                    print(f"  ⚠️ {framework_name}: Virtual environment warning missing")
                            else:
                                print(f"  ⚠️ {framework_name}: Example file not found")
                                frameworks_tested -= 1
        ...
        warning_rate = (correct_warnings / frameworks_tested) * 100 if frameworks_tested > 0 else 0
        ...
        assert warning_rate >= 80.0, f"Venv warning implementation rate too low: {warning_rate}%"
    ...
    def test_virtual_environment_detection_logic(self):
        ...
        for scenario in detection_scenarios:
            try:
                ...
                with patch.object(sys, 'real_prefix', scenario['real_prefix'], create=True):
                    with patch('sys.base_prefix', scenario['base_prefix']):
                        with patch('sys.prefix', scenario['prefix']):
                            ...
                            detected = (hasattr(sys, 'real_prefix') and sys.real_prefix is not None) or \
                            ...
                            if detected == scenario['expected']:
                                ...
                                print(f"  ❌ {scenario['name']}: Logic test failed - expected {scenario['expected']}, got {detected}")
        ...
        assert logic_accuracy >= 95.0, f"Venv detection logic accuracy too low: {logic_accuracy}%"
    ...
    def test_virtual_environment_guidance_messages(self):
            ...
            try:
                ...
                if example_file.exists():
                    ...
                    if guidance_score >= len(guidance_elements) - 2:
                        helpful_guidance += 1
                        print(f"  ✅ {framework_name}: Helpful virtual environment guidance provided")
                    else:
                        print(f"  ⚠️ {framework_name}: Virtual environment guidance could be improved")
                else:
                    print(f"  ⚠️ {framework_name}: Example file not found")
                    frameworks_tested -= 1
                    
            except Exception as e:
                print(f"  ⚠️ {framework_name}: Guidance analysis error - {str(e)[:50]}...")
        ...
        assert guidance_quality >= 75.0, f"Venv guidance quality too low: {guidance_quality}%"
    ...
    def test_cross_platform_venv_support(self):
            ...
            try:
                ...
                if example_file.exists():
                    ...
                    if platform_score >= 3:
                        ...
                        print(f"  ✅ {framework_name}: Cross-platform virtual environment support")
                    else:
                        print(f"  ⚠️ {framework_name}: Limited platform support for virtual environments")
                else:
                    print(f"  ⚠️ {framework_name}: Example file not found")
                    frameworks_tested -= 1
                    
            except Exception as e:
                print(f"  ⚠️ {framework_name}: Platform analysis error - {str(e)[:50]}...")
        
        platform_coverage = (cross_platform_support / frameworks_tested) * 100 if frameworks_tested > 0 else 0
        ...
        assert platform_coverage >= 70.0, f"Cross-platform venv coverage too low: {platform_coverage}%"
...
def test_virtual_environment_business_metrics():
    ...
    print(f"   • Frameworks with detection: {venv_metrics['frameworks_with_detection']}")
    print(f"   • Detection scenarios tested: {venv_metrics['detection_scenarios_tested']}")
    print(f"   • Cross-platform coverage: {venv_metrics['cross_platform_coverage']}%")
    
    print(f"\n🛡️ Conflict Prevention Impact:")
    print(f"   • Dependency conflict prevention: {venv_metrics['conflict_prevention_rate']}%")
