...
from adri.core.audit_logger_csv import CSVAuditLogger, AuditRecord
...
class TestAuditSelfValidation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.logger = CSVAuditLogger(config={
            "enabled": True,
    ...
    def test_audit_logs_self_validation(self):
        ...
        standards_dir = Path(__file__).parent.parent / "adri" / "standards" / "audit_logs"
        ...
        for log_type, result in validation_results.items():
            self.assertTrue(
                result["valid"],
                f"Audit log '{log_type}' failed ADRI validation"
            )
            self.assertGreaterEqual(
                result["score"],
                75.0,
    ...
    def test_log_rotation(self):
        ...
        small_logger = CSVAuditLogger(config={
            "enabled": True,
            "log_dir": self.test_dir,
