...
try:
    from tests.test_utils import OutputLimiter, skip_if_ci, TestDataGenerator
except ImportError:
...
class TestPerformanceMetrics:
    ...
    def test_protection_engine_small_dataset(
        self, generate_dataset, performance_monitor
    ):
        ...
        with patch.object(engine, "ensure_standard_exists"):
            with patch.object(engine, "assess_data_quality") as mock_assess:
                ...
                assert (
                    duration < self.SMALL_DATASET_MAX_TIME
                ), f"Small dataset took {duration:.3f}s, max allowed: {self.SMALL_DATASET_MAX_TIME}s"
                assert result == 1000

    def test_protection_engine_medium_dataset(
        self, generate_dataset, performance_monitor
    ):
        ...
        with patch.object(engine, "ensure_standard_exists"):
            with patch.object(engine, "assess_data_quality") as mock_assess:
                ...
                assert (
                    duration < self.MEDIUM_DATASET_MAX_TIME
                ), f"Medium dataset took {duration:.3f}s, max allowed: {self.MEDIUM_DATASET_MAX_TIME}s"
                assert result == 50000

    def test_protection_engine_large_dataset(
        self, generate_dataset, performance_monitor
    ):
        ...
        with patch.object(engine, "ensure_standard_exists"):
            with patch.object(engine, "assess_data_quality") as mock_assess:
                ...
                assert (
                    duration < self.LARGE_DATASET_MAX_TIME
                ), f"Large dataset took {duration:.3f}s, max allowed: {self.LARGE_DATASET_MAX_TIME}s"
        ...
        with patch(
            ...
            with patch.object(engine, "ensure_standard_exists"):
                with patch.object(engine, "assess_data_quality") as mock_assess:
                    ...
                    assert (
                        hit_rate >= self.CACHE_HIT_MIN_RATE
                    ), f"Cache hit rate {hit_rate:.2%} below minimum {self.CACHE_HIT_MIN_RATE:.2%}"
        ...
        with patch(
            ...
            with patch.object(engine, "ensure_standard_exists"):
                ...
                with patch.object(
                    engine,
                    "assess_data_quality",
                    wraps=lambda d, s: assessment_engine.assess(d, s),
                ):
                    engine.protect_function_call(
                        func=process_data,
                        args=(data,),
    ...
    def test_concurrent_operations_thread_safety(
        self, generate_dataset, performance_monitor
    ):
        ...
        scaling_efficiency = (
            expected_sequential_time / concurrent_time if concurrent_time > 0 else 1
        )

        # We expect at least 80% scaling efficiency
        assert (
            scaling_efficiency >= self.CONCURRENT_SCALING_MIN or concurrent_time < 0.5
    ...
    def test_audit_logger_write_performance(self, performance_monitor, tmp_path):
        ...
        config = {"enabled": True, "log_location": str(log_file)}
    ...
    def test_memory_efficiency_large_dataset(self, generate_dataset):
        ...
        assert (
            memory_growth < self.MEMORY_GROWTH_MAX_FACTOR
    ...
    def test_empty_dataset_performance(self, performance_monitor):
        ...
        with patch.object(engine, "ensure_standard_exists"):
            with patch.object(engine, "assess_data_quality") as mock_assess:
                mock_assess.return_value = Mock(
                    overall_score=0.0, passed=False, dimension_scores={}
                )
    ...
    def test_wide_dataset_performance(self, generate_dataset, performance_monitor):
        ...
        with patch.object(engine, "ensure_standard_exists"):
            with patch.object(engine, "assess_data_quality") as mock_assess:
                ...
                assert (
                    duration < self.SMALL_DATASET_MAX_TIME * 2
                ), f"Wide dataset took {duration:.3f}s"
    ...
    def test_generate_performance_report(
        self, generate_dataset, performance_monitor, tmp_path
    ):
    ...
    def test_batch_processing_efficiency(self, generate_dataset):
        ...
        with patch.object(engine, "ensure_standard_exists"):
            with patch.object(engine, "assess_data_quality") as mock_assess:
                ...
                assert (
                    efficiency_gain > 2.0
                ), f"Batch processing only {efficiency_gain:.1f}x faster"
...
class TestPerformanceRegression:
    ...
    def test_small_dataset_regression(self, generate_dataset):
        ...
        assert (
            avg_time < self.REGRESSION_THRESHOLDS["small_dataset_assessment"]
    ...
    def test_cache_lookup_regression(self):
        ...
        avg_time = sum(times) / len(times)
        assert (
            avg_time < self.REGRESSION_THRESHOLDS["cache_lookup"]
    ...
    def test_audit_write_regression(self, tmp_path):
        ...
        avg_time = sum(times) / len(times)
        assert (
            avg_time < self.REGRESSION_THRESHOLDS["audit_log_write"]
    ...
    def test_standard_loading_regression(self, tmp_path):
        ...
        avg_time = sum(times) / len(times)
        assert (
            avg_time < self.REGRESSION_THRESHOLDS["standard_loading"]
...
class TestPerformanceSummary:
    ...
    def test_generate_performance_summary(self, generate_dataset, tmp_path):
        ...
        # Check thresholds
        results["thresholds_met"]["small_dataset"] = (
            results["performance_metrics"]["1000_rows"]["time_seconds"] < 0.1
        )
        results["thresholds_met"]["medium_dataset"] = (
            results["performance_metrics"]["10000_rows"]["time_seconds"] < 0.5
        )
        results["thresholds_met"]["large_dataset"] = (
            results["performance_metrics"]["100000_rows"]["time_seconds"] < 2.0
        )

        # Add recommendations
        if not all(results["thresholds_met"].values()):
            results["recommendations"].append(
                "Consider optimizing assessment algorithms"
            )
        ...
        for size, metrics in results["performance_metrics"].items():
            print(
                f"{size}: {metrics['time_seconds']:.3f}s ({metrics['rows_per_second']:.0f} rows/sec)"
            )
        print("\nThresholds Met:")
        ...
        assert (
            sum(results["thresholds_met"].values())
            >= len(results["thresholds_met"]) * 0.8
