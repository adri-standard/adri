...
try:
    from tests.test_utils import TestDataGenerator, skip_if_ci
except ImportError:
...
class TestStressPerformance:
    ...
    def generate_extreme_dataset(self):
        ...
        def _generate(rows: int, cols: int = 10, sparse: bool = False, nested: bool = False):
            ...
            if sparse:
                ...
                data = {
                    ...
                    'categories': np.random.choice(['A'] * 100, rows),  # Low cardinality
                }
    ...
    def test_stress_maximum_concurrent_operations(self, generate_extreme_dataset):
        ...
        assert successful_operations >= num_threads * 0.95, \
    ...
    def test_stress_memory_leak_detection(self, generate_extreme_dataset):
        ...
        assert memory_growth < 0.2, f"Memory grew by {memory_growth:.1%} indicating potential leak"
    ...
    def test_stress_cache_overflow(self):
        ...
        with patch('adri.config.manager.ConfigManager.get_protection_config') as mock_config:
            ...
            engine._assessment_cache["new_key"] = (Mock(overall_score=85.0), time.time())
    ...
    def test_stress_recovery_from_errors(self):
        ...
        assert success_count >= 80, f"Only {success_count} operations succeeded after errors"
...
class TestPerformanceUnderLoad:
    ...
    def test_sustained_load_performance(self, generate_extreme_dataset):
        ...
        assert p95_response_time < 0.2, f"P95 response time {p95_response_time:.3f}s too high"
        assert p99_response_time < 0.5, f"P99 response time {p99_response_time:.3f}s too high"
        assert operation_count >= 100, f"Only {operation_count} operations in {test_duration}s"
