    ...
    def test_complete_workflow_with_cli(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert Path(
                    "adri-config.yaml"
                ).exists(), "Config file should be created"
                assert Path(
                    "ADRI/dev/training-data"
                ).exists(), "Training data directory should be created"
                assert Path(
                    "ADRI/dev/standards"
                ).exists(), "Standards directory should be created"
                assert Path(
                    "ADRI/dev/assessments"
                ).exists(), "Assessments directory should be created"
                ...
                assert (
                    result.returncode == 0
                ), f"Generate standard failed: {result.stderr}"

                # Check that standard file was created
                standard_file = Path(
                    "ADRI/dev/standards/sample_data_ADRI_standard.yaml"
                )
                ...
                assessment_files = list(
                    Path("ADRI/dev/assessments").glob(
                        "sample_data_sample_data_ADRI_standard_assessment_*.json"
                    )
                )
    ...
    def test_yaml_standard_structure_validation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                shutil.move(
                    str(csv_file), "ADRI/dev/training-data/comprehensive_data.csv"
                )
                ...
                standard_file = Path(
                    "ADRI/dev/standards/comprehensive_data_ADRI_standard.yaml"
                )
                with open(standard_file, "r") as f:
                ...
                for dimension in expected_dimensions:
                    ...
                    assert isinstance(
                        dim_reqs[dimension]["minimum_score"], (int, float)
                    )
    ...
    def test_assessment_score_validation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                high_quality_data = [
                    {
                        ...
                        "age": 25 + (i % 40),
                        "registration_date": f"2023-{1 + (i % 12):02d}-{1 + (i % 28):02d}",
                ...
                shutil.move(
                    str(csv_file), "ADRI/dev/training-data/high_quality_data.csv"
                )
                ...
                for line in lines:
                    if "Overall Score:" in line:
                        score_part = (
                            line.split("Overall Score:")[1].split("/")[0].strip()
                        )
                ...
                assert (
                    overall_score >= 80.0
                ), f"High-quality data should score well, got {overall_score}"
                assert (
                    overall_score <= 100.0
                ), f"Score should not exceed 100, got {overall_score}"
                ...
                for line in lines:
                    for dimension in [
                        ...
                        if f"{dimension}:" in line:
                            score_part = (
                                line.split(f"{dimension}:")[1].split("/")[0].strip()
                            )
