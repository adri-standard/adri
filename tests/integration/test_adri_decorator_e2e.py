...
class TestADRIDecoratorE2E:
    ...
    def test_no_standard_auto_generate_true(self, excellent_data, temp_standard_dir):
        ...
        with patch('adri.config.manager.ConfigManager.get_protection_config') as mock_config:
            ...
            with patch('adri.config.manager.ConfigManager.resolve_standard_path_simple') as mock_path:
                ...
                with patch.object(engine.standards_loader, 'standard_exists', return_value=False):
                    # Mock the assessment to return good score
                    with patch.object(engine, 'assess_data_quality') as mock_assess:
                        mock_assess.return_value = Mock(overall_score=95.0, passed=True,
                                                       dimension_scores={})
    ...
    def test_file_based_standard_exists(self, excellent_data, temp_standard_dir):
        ...
        with patch('adri.config.manager.ConfigManager.resolve_standard_path_simple') as mock_path:
            ...
            with patch.object(engine, 'assess_data_quality') as mock_assess:
                mock_assess.return_value = Mock(overall_score=85.0, passed=True,
                                               dimension_scores={})
    ...
    def test_bundled_standard_exists(self, excellent_data, sample_standard_dict):
        ...
        with patch.object(engine.standards_loader, 'standard_exists') as mock_exists:
            with patch.object(engine.standards_loader, 'load_standard') as mock_load:
                ...
                with patch.object(engine, 'assess_data_quality') as mock_assess:
                    mock_assess.return_value = Mock(overall_score=90.0, passed=True,
                                                   dimension_scores={})
    ...
    def test_both_bundled_and_file_standard_exist(self, excellent_data, temp_standard_dir, sample_standard_dict):
        ...
        with patch.object(engine.standards_loader, 'standard_exists', return_value=True):
            with patch.object(engine.standards_loader, 'load_standard') as mock_load:
            ...
            with patch.object(engine, 'assess_data_quality') as mock_assess:
                ...
                with pytest.raises(ProtectionError) as exc_info:
                    engine.protect_function_call(
                        ...
                        function_name='process_data',
                        min_score=80.0,
                        dimensions={'validity': 15.0, 'completeness': 15.0},  # Completeness will fail
                        on_failure='raise'
                    )
    ...
    def test_successful_assessment_logged(self, excellent_data, mock_assessment_result):
        ...
        with patch.object(engine, 'ensure_standard_exists'):
            with patch.object(engine, 'assess_data_quality') as mock_assess:
                mock_assess.return_value = mock_assessment_result(score=90.0, passed=True)
    ...
    def test_failed_assessment_logged(self, bad_data, mock_assessment_result):
        ...
        with patch.object(engine, 'ensure_standard_exists'):
            with patch.object(engine, 'assess_data_quality') as mock_assess:
                mock_assess.return_value = mock_assessment_result(score=40.0, passed=False)
    ...
    def test_audit_logging_failure_continues(self, excellent_data, mock_assessment_result):
            ...
            with patch.object(engine, 'assess_data_quality') as mock_assess:
                ...
                with patch.object(engine.audit_logger, 'log_assessment') as mock_log:
                    ...
                    with patch('adri.core.protection.logger') as mock_logger:
                        ...
                        mock_logger.warning.assert_called()
                        assert "Failed to create audit log" in str(mock_logger.warning.call_args)
                        
                        # But function should still execute
                        assert result == "Success despite audit failure"
    
    def test_concurrent_assessments_logged(self, excellent_data, mock_assessment_result):
        ...
        def run_protected_function(thread_id):
            try:
                with patch.object(engine, 'ensure_standard_exists'):
                    with patch.object(engine, 'assess_data_quality') as mock_assess:
                        mock_assess.return_value = mock_assessment_result(score=85.0 + thread_id)
    ...
    def test_empty_dataframe(self, mock_assessment_result):
        ...
        with patch.object(engine.standards_loader, 'standard_exists', return_value=False):
            with patch.object(engine, 'ensure_standard_exists'):
                with patch.object(engine, 'assess_data_quality') as mock_assess:
                    mock_assess.return_value = mock_assessment_result(score=50.0)  # Low score for empty
    ...
    def test_default_configuration(self, excellent_data, mock_assessment_result):
        ...
        with patch('adri.config.manager.ConfigManager.get_protection_config') as mock_config:
            ...
            with patch.object(engine, 'ensure_standard_exists'):
                with patch.object(engine, 'assess_data_quality') as mock_assess:
                    mock_assess.return_value = mock_assessment_result(score=76.0)  # Just above default
    ...
    def test_repeated_assessment_uses_cache(self, excellent_data, mock_assessment_result):
        ...
        with patch('adri.config.manager.ConfigManager.get_protection_config') as mock_config:
            ...
            with patch.object(engine, 'ensure_standard_exists'):
                with patch.object(engine, 'assess_data_quality', wraps=engine.assess_data_quality) as mock_assess:
                    ...
                    with patch.object(engine, '_generate_data_hash') as mock_hash:
                        ...
                        with patch.object(engine, 'resolve_standard') as mock_resolve:
                            ...
                            engine._assessment_cache["test_standard.yaml:test_hash"] = (cached_result, time.time())
    ...
    def test_e2e_real_scenario_auto_generate_and_assess(self, temp_standard_dir):
        ...
        real_data = pd.DataFrame({
            ...
            'email': ['john@example.com', 'jane@test.org', 'bob@company.net', 
                     'alice@domain.com', 'charlie@email.org'],
            'age': [25, 30, 35, 28, 45],
        ...
        with patch('adri.config.manager.ConfigManager.get_protection_config') as mock_config:
            ...
            with patch('adri.config.manager.ConfigManager.resolve_standard_path_simple') as mock_path:
                standard_path = os.path.join(temp_standard_dir, 'customer_standard.yaml')
                mock_path.return_value = standard_path
            ...
            with patch('adri.config.manager.ConfigManager.resolve_standard_path_simple') as mock_path:
                ...
                with patch.object(engine, 'assess_data_quality') as mock_assess:
                    ...
                    with patch.object(engine.audit_logger, 'log_assessment') as mock_log:
                        ...
                        assert audit_call['execution_context']['assessment_passed'] is False
