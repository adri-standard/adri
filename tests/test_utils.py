...
class OutputLimiter:
    ...
    def _truncate_dataframe(cls, df: pd.DataFrame, context: str) -> str:
        ...
        return (f"{shape_info} - showing first and last {cls.MAX_DATAFRAME_ROWS // 2} rows\n"
                f"{head}\n...\n{tail}")
...
class TestDataGenerator:
    ...
    @staticmethod
    def generate_dataset(rows: int, cols: int = 10, 
                         test_type: str = "normal") -> pd.DataFrame:
        """
        ...
        if safe_rows < rows:
            print(f"⚠️  Dataset reduced from {rows} to {safe_rows} rows for {test_type} test")
        
        data = {
            f'col_{i}': np.random.randn(safe_rows) 
            for i in range(cols)
        }
