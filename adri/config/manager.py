...
class ConfigManager:
    ...
    def validate_config(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            required_fields = ['version', 'project_name', 'environments', 'default_environment']
    ...
    def find_config_file(self, start_path: str = '.') -> Optional[str]:
        ...
        for path in [current_path] + list(current_path.parents):
            ...
            fallback_names = ['adri-config.yaml', 'adri-config.yml', '.adri.yaml', '.adri.yml']
    ...
    def get_environment_config(self, config: Dict[str, Any], environment: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def resolve_standard_path(self, standard_name: str, config: Dict[str, Any], environment: Optional[str] = None) -> str:
        """
    ...
    def get_assessments_dir(self, config: Dict[str, Any], environment: Optional[str] = None) -> str:
        """
    ...
    def get_training_data_dir(self, config: Dict[str, Any], environment: Optional[str] = None) -> str:
        """
        ...
        for env_name, env_config in adri_config['environments'].items():
            ...
            for path_type, path_value in paths.items():
                ...
                status = {
                    ...
                    'readable': os.access(path_value, os.R_OK) if path_obj.exists() else False,
                    'writable': os.access(path_value, os.W_OK) if path_obj.exists() else False,
                    'file_count': 0
                }
                ...
                if not status['exists']:
                    results['warnings'].append(f"Path does not exist: {path_value} ({env_name}.{path_type})")
                elif not status['is_directory']:
                    results['errors'].append(f"Path is not a directory: {path_value} ({env_name}.{path_type})")
                    results['valid'] = False
                elif not status['readable']:
                    results['errors'].append(f"Path is not readable: {path_value} ({env_name}.{path_type})")
                    results['valid'] = False
                elif not status['writable']:
                    results['warnings'].append(f"Path is not writable: {path_value} ({env_name}.{path_type})")
    ...
    def resolve_standard_path_simple(self, standard_name: str, environment: Optional[str] = None) -> str:
        """
