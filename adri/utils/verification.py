...
def verify_standalone_installation() -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        if 'adri-standards' in installed_packages:
            success = False
            messages.append("❌ External adri-standards package detected - not standalone")
        else:
            messages.append("✅ No external adri-standards dependency found")
    except ImportError:
        messages.append("⚠️  Could not verify package dependencies (pkg_resources not available)")
...
def run_full_verification(verbose: bool = True) -> bool:
    ...
    if verbose:
        ...
        print(f"   Packages Compatible: {'✅' if sys_info['packages_compatible'] else '❌'}")
        
        if sys_info['missing_packages']:
            print(f"   Missing Packages: {', '.join(sys_info['missing_packages'])}")
    
    all_success = all_success and sys_info['python_compatible'] and sys_info['packages_compatible']
