...
def assess_command(
    ...
    try:
        ...
        if output_path is None:
            ...
            output_path = os.path.join(env_config['paths']['assessments'], output_filename)
        ...
        if len(data) > max_rows:
            print(f"⚠️  Warning: Dataset has {len(data):,} rows, limiting to {max_rows:,} rows per configuration")
        ...
        if verbose:
            ...
            for dim, score in assessment.dimension_scores.items():
                print(f"  {dim.title()}: {score.score:.1f}/20.0 ({score.percentage():.1f}%)")
        else:
...
def generate_adri_standard_command(
    ...
    try:
        ...
        if len(data) > max_rows:
            if verbose:
                print(f"⚠️  Warning: Dataset has {len(data):,} rows, analyzing first {max_rows:,} rows per configuration")
            data = data.head(max_rows)
        ...
        # Save to YAML file
        with open(output_path, 'w') as f:
            yaml.dump(standard_dict, f, default_flow_style=False, sort_keys=False, indent=2)
        ...
        if verbose:
            ...
            print(f"  Field Requirements: {len(requirements['field_requirements'])} fields")
            print(f"  Dimension Requirements: {len(requirements['dimension_requirements'])} dimensions")
        
        print(f"\n📋 Next steps:")
        print(f"1. Review the generated standard: {output_path}")
        print(f"2. Run assessment: adri assess {data_path} --standard {output_filename}")
...
def validate_standard_command(
    ...
    try:
        ...
        if validation_result["is_valid"]:
            ...
            print(f"📄 Standard: {validation_result['standard_name']} v{validation_result['standard_version']}")
...
def validate_yaml_standard(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(yaml_content, dict):
            validation_result["errors"].append("YAML must contain a dictionary at the root level")
            validation_result["is_valid"] = False
            return validation_result
        ...
        if "requirements" in yaml_content:
            _validate_requirements_section(yaml_content["requirements"], validation_result)
        ...
        try:
            ...
            validation_result["passed_checks"].append("YAMLStandards instantiation successful")
        except Exception as e:
...
def _validate_standards_metadata(standards_section: Dict[str, Any], result: Dict[str, Any]):
    """Validate the standards metadata section."""
    required_fields = ["id", "name", "version", "authority"]
    
    for field in required_fields:
        if field not in standards_section:
            result["errors"].append(f"Missing required field in standards section: '{field}'")
            ...
            result["passed_checks"].append(f"Standards field '{field}' present and non-empty")
    
    # Validate version format (basic semver check)
    if "version" in standards_section:
        version = str(standards_section["version"])
        if not re.match(r'^\d+\.\d+\.\d+', version):
            result["warnings"].append(f"Version '{version}' does not follow semantic versioning (x.y.z)")
    ...
    if "effective_date" in standards_section:
        try:
            ...
            result["errors"].append(f"Invalid effective_date format: {standards_section['effective_date']} (expected ISO format)")
            result["is_valid"] = False


def _validate_requirements_section(requirements_section: Dict[str, Any], result: Dict[str, Any]):
    ...
    if "dimension_requirements" in requirements_section:
        _validate_dimension_requirements(requirements_section["dimension_requirements"], result)
...
def _validate_dimension_requirements(dim_requirements: Dict[str, Any], result: Dict[str, Any]):
    ...
    for dimension, config in dim_requirements.items():
        if dimension not in valid_dimensions:
            result["errors"].append(f"Unknown dimension: '{dimension}'. Valid dimensions: {valid_dimensions}")
            result["is_valid"] = False
        else:
        ...
        if isinstance(config, dict):
            if "minimum_score" in config:
                ...
                if not isinstance(min_score, (int, float)):
                    result["errors"].append(f"minimum_score for {dimension} must be a number")
                    result["is_valid"] = False
                elif min_score < 0 or min_score > 20:
                    result["errors"].append(f"minimum_score for {dimension} must be between 0 and 20")
                    result["is_valid"] = False
                else:
                    result["passed_checks"].append(f"Valid minimum_score for {dimension}: {min_score}")


def _validate_field_requirements(field_requirements: Dict[str, Any], result: Dict[str, Any]):
    ...
    for field_name, field_config in field_requirements.items():
        if not isinstance(field_config, dict):
            result["errors"].append(f"Field '{field_name}' configuration must be a dictionary")
            result["is_valid"] = False
            continue
        ...
        if "type" in field_config:
            ...
            if field_type not in valid_types:
                result["errors"].append(f"Invalid type '{field_type}' for field '{field_name}'. Valid types: {valid_types}")
                result["is_valid"] = False
            else:
                result["passed_checks"].append(f"Valid type for field '{field_name}': {field_type}")
        
        # Validate nullable
        if "nullable" in field_config:
            nullable = field_config["nullable"]
            if not isinstance(nullable, bool):
                result["errors"].append(f"nullable for field '{field_name}' must be true or false")
                result["is_valid"] = False
            else:
                result["passed_checks"].append(f"Valid nullable setting for field '{field_name}': {nullable}")
        ...
        if "min_value" in field_config and "max_value" in field_config:
            ...
            if isinstance(min_val, (int, float)) and isinstance(max_val, (int, float)):
                if min_val >= max_val:
                    result["errors"].append(f"min_value must be less than max_value for field '{field_name}'")
                    result["is_valid"] = False
                else:
                    result["passed_checks"].append(f"Valid min/max range for field '{field_name}': {min_val}-{max_val}")
        ...
        if "pattern" in field_config:
            ...
            try:
                re.compile(pattern)
                result["passed_checks"].append(f"Valid regex pattern for field '{field_name}'")
            except re.error as e:
                result["errors"].append(f"Invalid regex pattern for field '{field_name}': {e}")
                result["is_valid"] = False
...
def show_config_command(
    ...
    try:
        ...
        if format_type == "json":
            ...
            return _output_config_human(config, environment, paths_only, validate, validation_results, config_manager)
...
def _output_config_json(
    ...
    output = {
        "config": config,
        "active_environment": environment or config['adri'].get('default_environment', 'development')
    }
...
def _output_config_human(
    ...
    environments_to_show = [environment] if environment else list(adri_config['environments'].keys())
...
def _show_validation_results(validation_results: Dict[str, Any]):
    ...
    # Show file counts
    total_files = sum(status['file_count'] for status in path_status.values() 
                     if status['file_count'] > 0)
    if total_files > 0:
                    ...
                    try:
                        if ext == '.csv':
                            ...
                            try:
                                ...
                                print(f"    📊 Format: Parquet (pandas required for details)")
...
def list_assessments_command(
    ...
    try:
        ...
        print("  • Compare: adri assess <data> --standard <standard> (creates new assessment)")
...
def export_report_command(
    ...
    try:
        ...
        if format_type == "json":
            ...
            rows.append(["Overall Score", f"{assessment_data.get('overall_score', 'N/A')}/100"])
            rows.append(["Status", "PASSED" if assessment_data.get('passed', False) else "FAILED"])
            ...
            if 'dimension_scores' in assessment_data:
                for dim_name, dim_data in assessment_data['dimension_scores'].items():
                    if isinstance(dim_data, dict) and 'score' in dim_data:
                        ...
                        rows.append([dim_name.title(), f"{score:.1f}", "20.0", f"{percentage:.1f}%"])
...
def show_standard_command(
    ...
    try:
        ...
        # Usage instructions
        print("📋 Usage:")
        data_name = Path(standard_path).stem.replace('_standard', '').replace('_ADRI_standard', '')
        print(f"  • Test data: adri assess <data-file> --standard {Path(standard_path).name}")
...
def explain_failure_command(
    ...
    try:
        ...
        if latest:
            ...
            print("❌ Error: Must specify either --latest or provide assessment file")
            print("💡 Use 'adri explain-failure --latest' or 'adri explain-failure <file>'")
            return 1
        ...
        for dim_name, dim_data in dimension_scores.items():
            if isinstance(dim_data, dict) and 'score' in dim_data:
                ...
                print(f"  {status} {dim_name.title()}: {score:.1f}/20 ({percentage:.1f}%)")
        ...
        if failed_dimensions:
            ...
            for dim_name, score, dim_data in failed_dimensions:
                ...
                recommendations = _get_dimension_recommendations(dim_name, score, dim_data)
                if recommendations:
                    print("   💡 How to fix:")
...
def _get_dimension_recommendations(dim_name: str, score: float, dim_data: dict) -> List[str]:
    ...
    if score < 5:
        recommendations.insert(0, f"CRITICAL: {dim_name} has severe issues - major data cleanup needed")
    elif score < 10:
        recommendations.insert(0, f"MAJOR: {dim_name} needs significant improvement")
    else:
        recommendations.insert(0, f"MINOR: {dim_name} needs some cleanup to meet standards")
...
def main():
    ...
    def assess(data_path, standard_path, output_path, verbose, environment, config_path):
        """Run data quality assessment."""
        return assess_command(data_path, standard_path, output_path, verbose, environment, config_path)
    ...
    def generate_standard(data_path, force, verbose, environment, config_path):
        """Generate ADRI standard from data file analysis."""
        return generate_adri_standard_command(data_path, force, verbose, environment, config_path)
