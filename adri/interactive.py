...
This module provides an interactive, guided experience for running ADRI assessments.
"""
...
def validate_file_path(_, current):
    """Validate that the file path exists."""
    if not os.path.exists(current):
        raise errors.ValidationError("", reason=f"File does not exist: {current}")
    return current


def validate_output_path(_, current):
...
def run_interactive_mode():
    ...
    This function guides the user through the process of assessing a data source
    ...
    if source_type == "file":
        ...
        try:
            if source_path.endswith(".csv"):
                ...
                print(f"\nFile detected: CSV with {len(df)} rows and {len(df.columns)} columns.")
            elif source_path.endswith(".json"):
                df = pd.read_json(source_path)
                print(f"\nFile detected: JSON with {len(df)} rows and {len(df.columns)} columns.")
            else:
    ...
    dimension_choices = [
        (f"{dim} - {dimension_descriptions.get(dim, '')}", dim)
    ...
    if customize:
        for dim in selected_dimensions:
            questions = [
                inquirer.Text(
                    "weight",
                    message=f"Weight for {dim} dimension (0.5-2.0, default is 1.0):",
                    default="1.0",
                    validate=lambda _, x: 0.5 <= float(x) <= 2.0,
                ),
                inquirer.Text(
                    "threshold",
                    message=f"Threshold for {dim} dimension (0.0-1.0, default varies):",
                    default="0.8",
    ...
    if "html" in output_formats:
        html_path = f"{output_path}.html" if not output_path.endswith(".html") else output_path
    ...
    while True:
        questions = [
            inquirer.List(
                ...
                choices=[
                    ("View detailed results for a specific dimension", "view_dimension"),
        ...
        if next_action == "exit":
            ...
            questions = [
                inquirer.List(
                    ...
                    choices=[(dim, dim) for dim in report.dimension_results.keys()],
                ),
            ]
            answers = inquirer.prompt(questions)
