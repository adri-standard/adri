...
class DataProfiler:
    ...
    def _create_summary(self, data: pd.DataFrame, original_rows: int, analyzed_rows: int) -> Dict[str, Any]:
    ...
    def _profile_boolean_field(self, series: pd.Series) -> Dict[str, Any]:
        ...
        return {
            ...
            'false_count': int(len(non_null_series) - (value_counts.get(True, 0) + 
    ...
    def _detect_string_pattern(self, series: pd.Series) -> Optional[str]:
        ...
        if len(series) > 0 and all(self.email_pattern.match(str(val)) for val in series):
