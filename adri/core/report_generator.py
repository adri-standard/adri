...
class ReportGenerator:
    ...
    def generate_report(self, assessment_result) -> Dict[str, Any]:
        ...
        report = {
            "adri_assessment_report": {
                "metadata": self._build_metadata(assessment_result, assessment_id, timestamp),
    ...
    def _build_metadata(self, assessment_result, assessment_id: str, timestamp: datetime) -> Dict[str, Any]:
    ...
    def _build_summary(self, assessment_result) -> Dict[str, Any]:
        ...
        if hasattr(assessment_result, 'dimension_scores') and assessment_result.dimension_scores is not None:
            ...
            if hasattr(dim_scores, '__dict__'):
                ...
                for dim_name, dim_value in dim_scores.items():
                    if hasattr(dim_value, 'score'):
                        ...
                        dimension_scores[dim_name] = float(dim_value) if isinstance(dim_value, (int, float)) else 0.0
        ...
        if hasattr(assessment_result, 'dimension_scores'):
            ...
            for dimension in dimensions:
                ...
                rule_entry = {
                    ...
                    "execution": {
                        "total_records": getattr(assessment_result, 'total_records', 0),
                        "passed": int((score / 20.0) * getattr(assessment_result, 'total_records', 0)),
                        "failed": int(((20.0 - score) / 20.0) * getattr(assessment_result, 'total_records', 0)),
                        "rule_score": score,
                        "rule_weight": 1.0
                    }
                }
    ...
    def _build_field_analysis(self, assessment_result) -> Dict[str, Any]:
        ...
        if hasattr(assessment_result, 'field_analysis') and assessment_result.field_analysis:
            ...
            for field_name, field_data in assessment_result.field_scores.items():
                if field_name not in field_analysis:
                    field_analysis[field_name] = {
                        "rules_applied": getattr(field_data, 'rules_applied', [f"{field_name}_validation"]),
                        "overall_field_score": getattr(field_data, 'score', 0.0),
                        "total_failures": getattr(field_data, 'failures', 0),
                        "ml_readiness": self._assess_ml_readiness(getattr(field_data, 'score', 0.0)),
                        "recommended_actions": []
                    }
    ...
    def validate_report(self, report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "adri_assessment_report" not in report:
            validation_results["errors"].append("Missing root object 'adri_assessment_report'")
        ...
        required_sections = self.template["field_requirements"]["adri_assessment_report"]["required_fields"]
