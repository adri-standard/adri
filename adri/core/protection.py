...
class DataProtectionEngine:
    ...
    def resolve_standard(
        ...
        bundled_standard = self._try_bundled_standard(function_name, data_param, standard_file, standard_name)
        ...
        pattern = self.protection_config.get(
            ...
            '{function_name}_{data_param}_standard.yaml'
        )
        ...
        try:
            ...
            if standard_file:
                ...
                if self.bundled_loader.standard_exists(base_name):
                    logger.debug(f"Found bundled standard for explicit file: {base_name}")
            ...
            for name in potential_names:
                if self.bundled_loader.standard_exists(name):
                    logger.debug(f"Found bundled standard with pattern matching: {name}")
                    return self.bundled_loader.load_standard(name)
            
            logger.debug("No matching bundled standard found, falling back to file-based standards")
            return None
            
        except StandardNotFoundError:
            logger.debug("Bundled standard not found, falling back to file-based standards")
            return None
        except Exception as e:
            logger.warning(f"Error trying bundled standards: {e}, falling back to file-based standards")
            return None
    
    def ensure_standard_exists(self, standard: Union[str, Dict], sample_data: Any) -> bool:
        """
        ...
        try:
            ...
            if len(df) > sampling_limit:
                logger.info(f"Sampling {sampling_limit} rows from {len(df)} total rows for standard generation")
            ...
            data_name = os.path.basename(standard_path).replace('_standard.yaml', '').replace('.yaml', '')
            
            # Get generation configuration from protection config
            generation_config = self.protection_config.get('generation', {
                'default_thresholds': {
                    'completeness_min': 85,
                    'validity_min': 90,
                    'consistency_min': 80
    ...
    def assess_data_quality(self, data: Any, standard: Union[str, Dict]) -> Any:
        ...
        if isinstance(standard, dict):
            logger.debug(f"Running fresh assessment with bundled standard: {standard.get('standards', {}).get('id', 'unknown')}")
        else:
    ...
    def handle_quality_failure(
        ...
        """
        error_message = self._format_quality_error(assessment_result, min_score, standard_path)
        ...
        if failure_mode == "raise":
            ...
            logger.error(f"Invalid failure mode '{failure_mode}', defaulting to 'raise'")
            raise ProtectionError(error_message)
    ...
    def protect_function_call(
        ...
        on_failure = on_failure if on_failure is not None else self.protection_config.get('default_failure_mode', 'raise')
        verbose = verbose if verbose is not None else self.protection_config.get('verbose_protection', False)
        
        if verbose:
            logger.info(f"Protecting function '{function_name}' with min_score={min_score}")
        
        # Resolve standard (could be file path or bundled standard dict)
        standard = self.resolve_standard(function_name, data_param, standard_file, standard_name)
        ...
        if verbose:
            logger.info(f"Assessment score: {assessment_result.overall_score}/{min_score}")
        
        # Check overall score
        if assessment_result.overall_score < min_score:
            self.handle_quality_failure(assessment_result, on_failure, min_score, standard)
        
        # Check dimension-specific requirements
        if dimensions:
            self._check_dimension_requirements(assessment_result, dimensions, on_failure)
    ...
    def _format_quality_error(
        ...
        error_lines.extend([
            ...
            "   • Test your fixes: adri assess <your-data> --standard " + standard_name
        ])
        
        return "\n".join(error_lines)
    
    def _identify_main_issues(self, assessment_result: Any, problem_dimensions: list) -> str:
    ...
    def _format_quality_success(
            ...
            if os.path.exists(standard):
                try:
                    ...
                    with open(standard, 'r') as f:
                        ...
                        if 'standards' in standard_data and 'version' in standard_data['standards']:
        ...
        if verbose:
            return self._format_verbose_success(
                assessment_result, min_score, standard_name, standard_version, 
                standard_created, standard if isinstance(standard, str) else None
            )
        else:
