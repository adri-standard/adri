...
class DataLoader:
    ...
    def load_json(self, file_path: str, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Successfully loaded JSON with {len(df)} rows and {len(df.columns)} columns")
            return df
    ...
    def load_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        logger.debug(f"Loading DataFrame with {len(df)} rows and {len(df.columns)} columns")
