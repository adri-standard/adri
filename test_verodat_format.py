    ...
    for i, expected_field in enumerate(schema["target_fields"]):
        if i < len(header):
            ...
            if actual_type == expected_verodat_type:
                ...
                print(
                    f"  ❌ {expected_field['name']}: expected {expected_verodat_type}, got {actual_type}"
                )
...
def main():
    ...
    results["assessment_logs"] = verify_payload_format(
        payload,
        next(
            ds for ds in schema["data_stores"] if ds["name"] == "adri_assessment_logs"
        ),
    )
    ...
    results["dimension_scores"] = verify_payload_format(
        payload,
        next(
            ds for ds in schema["data_stores"] if ds["name"] == "adri_dimension_scores"
        ),
    )
    ...
    results["failed_validations"] = verify_payload_format(
        ...
        next(
            ds
            for ds in schema["data_stores"]
            if ds["name"] == "adri_failed_validations"
        ),
    )
