...
class TestDataLoaderCoverage:
    ...
    def test_load_csv_exception_handling(self):
        ...
        try:
            with patch(
                "pandas.read_csv", side_effect=RuntimeError("CSV loading failed")
            ):
    ...
    def test_load_json_manual_fallback(self):
        ...
        try:
            ...
            with patch(
                "pandas.read_json", side_effect=ValueError("pandas can't handle this")
            ):
    ...
    def test_load_from_dict_exception_handling(self):
        ...
        with patch(
            "pandas.DataFrame", side_effect=RuntimeError("DataFrame creation failed")
        ):
            with pytest.raises(RuntimeError):
...
class TestLoadDataCoverage:
    ...
    def test_load_data_loader_exception(self):
        ...
        try:
            ...
            with patch.object(
                DataLoader, "load_csv", side_effect=RuntimeError("CSV loading failed")
            ):
                with pytest.raises(RuntimeError):
                    load_data(csv_path)
        finally:
