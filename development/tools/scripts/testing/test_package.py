...
def test_package_structure(
    result: PackageTestResult, package_dir: Path, verbose: bool = False
):
    ...
    for file_path in required_files:
        ...
        if full_path.exists():
            ...
            result.add_fail(
                f"Missing file: {file_path}", f"File not found: {full_path}"
            )
    ...
    for dir_path in required_dirs:
        ...
        if full_path.exists() and full_path.is_dir():
            ...
            result.add_fail(
                f"Missing directory: {dir_path}", f"Directory not found: {full_path}"
            )
    ...
    for module_path in key_modules:
        ...
        if full_path.exists():
            ...
            result.add_fail(
                f"Missing module: {module_path}", f"Module not found: {full_path}"
            )
    ...
    try:
        ...
        if version_file.exists():
            ...
            if version_py:
                ...
                result.add_fail(
                    "Version extraction", "__version__ not found in version.py"
                )
        else:
            ...
            if "project" in config:
                ...
                for field in required_fields:
                    if field in project:
                        ...
                        result.add_fail(
                            f"pyproject.toml missing {field}",
                ...
                if "dependencies" in project:
                    ...
                    if has_standards:
                        ...
                        result.add_fail(
                            "Dependencies", "adri-standards dependency not found"
                        )
            else:
...
def test_dependencies(
    ...
    try:
        ...
        result.add_warning(
            "Dependencies", "adri-standards not available (may need to be installed)"
        )
    ...
    for dep_name, description in optional_deps:
        try:
            ...
            result.add_warning(
                f"Optional dependency: {dep_name}", f"{description} may not work"
            )
    ...
    try:
        ...
        try:
            ...
            if help_result.exit_code == 0:
                ...
                result.add_fail(
                    "CLI help command", f"Exit code: {help_result.exit_code}"
                )
...
def test_package_installation(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if standards_path.exists():
            success, stdout, stderr = run_command(
                [str(pip_exe), "install", "-e", str(standards_path)]
            )
            if success:
                result.add_pass("Standards dependency installation")
            else:
                result.add_warning(
                    "Standards dependency", "Could not install adri-standards"
                )

        # Install the validator package
        success, stdout, stderr = run_command(
            [str(pip_exe), "install", "-e", str(package_dir)]
        )
        if success:
...
def test_publishing_readiness(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Build package
            success, stdout, stderr = run_command(
                [sys.executable, "-m", "build", str(temp_package)], timeout=120
            )
            ...
            if success:
                ...
                if dist_dir.exists():
                    ...
                    if len(built_files) >= 2:  # Should have .tar.gz and .whl
                        ...
                        result.add_warning(
                            "Built files", f"Only {len(built_files)} artifacts built"
                        )
                else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test ADRI Validator package before publishing"
    )
